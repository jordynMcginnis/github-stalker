...
class Fan extends Component {
  render({props}) {
    ...
    const result = props.events.reduce((result, {item}) => {
    ...
    let personEvent = props.events.filter(({item}) => actor.login === name);
    return (
      <div className='Contributions'>
        ...
        <div className='fan-results'>
          {personEvent.map(({item}) => <div className='f-result' key={repo.name}> <span>{type}:</span>{repo.name}<span>{created_at.slice(5,7)}/{created_at.slice(8,10)}/{created_at.slice(0,4)}</span></div>)}
        </div>
      </div>
    );
  }
